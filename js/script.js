...
async function upgradeToPremium() {
    ...
    try {
         const res = await fetch('https://redactafacil.onrender.com/api/upgrade-premium', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        nombre: "RedactaFácil Premium",
        precio: 3.00
      })
    });
    ...
    window.location.href = data.init_point;
        ...
        if (response.ok) {
            ...
            showToast(data.error || 'Error al actualizar a premium', 'error');
        }
    } catch (error) {
